...
<main class="app-main" id="main-content">
  ...
  <div class="app-canvas">
    ...
    <?php
      ...
      $fDepto         = $filters['departamento_id'] ?? '';
      $fSort          = $filters['sort']          ?? 'desc';
      $categorias    ??= [];
      $departamentos ??= [];
      $isAdmin       ??= false;
      function tkUrl(array $extra = []): string {
        global $fEstado, $fPrioridad, $fQ, $fCat, $fDepto, $fSort;
        $base = ['controller'=>'Ticket','action'=>'listar',
                 'estado'=>$fEstado,'prioridad'=>$fPrioridad,
                 'q'=>$fQ,'categoria_id'=>$fCat,'departamento_id'=>$fDepto,'sort'=>$fSort];
    ...
    <form method="GET" action="index.php" id="tk-filter-form">
      ...
      <?php if ($fDepto):     ?><input type="hidden" name="departamento_id" value="<?= htmlspecialchars($fDepto) ?>"><?php endif; ?>
    <div class="tk-filter-bar">
      ...
      <div class="tk-filter-group">
        <a class="tk-filter-pill<?= $fEstado===''?       ' active':'' ?>" href="<?= tkUrl() ?>">Todos</a>
        <a class="tk-filter-pill<?= $fEstado==='sin_abrir'?' active':'' ?>" href="<?= tkUrl(['estado'=>'sin_abrir']) ?>">Sin abrir</a>
        <a class="tk-filter-pill<?= $fEstado==='abierta'?  ' active':'' ?>" href="<?= tkUrl(['estado'=>'abierta']) ?>">Abiertos</a>
        <a class="tk-filter-pill<?= $fEstado==='en_proceso'?' active':'' ?>" href="<?= tkUrl(['estado'=>'en_proceso']) ?>">En proceso</a>
        <a class="tk-filter-pill<?= $fEstado==='resuelta'?  ' active':'' ?>" href="<?= tkUrl(['estado'=>'resuelta']) ?>">Resueltos</a>
        <a class="tk-filter-pill<?= $fEstado==='cerrada'?   ' active':'' ?>" href="<?= tkUrl(['estado'=>'cerrada']) ?>">Cerrados</a>
      </div>
      <div class="tk-filter-sep"></div>
      <div class="tk-filter-group">
        <a class="tk-filter-pill<?= $fPrioridad==='critica'?' active p-high':   ' p-high'   ?>" href="<?= tkUrl(['prioridad'=>'critica','estado'=>$fEstado]) ?>">Crítica</a>
        <a class="tk-filter-pill<?= $fPrioridad==='alta'?   ' active p-high':   ' p-high'   ?>" href="<?= tkUrl(['prioridad'=>'alta',   'estado'=>$fEstado]) ?>">Alta</a>
        <a class="tk-filter-pill<?= $fPrioridad==='media'?  ' active p-medium':  ' p-medium'  ?>" href="<?= tkUrl(['prioridad'=>'media',  'estado'=>$fEstado]) ?>">Media</a>
        <a class="tk-filter-pill<?= $fPrioridad==='baja'?   ' active p-low':    ' p-low'    ?>" href="<?= tkUrl(['prioridad'=>'baja',   'estado'=>$fEstado]) ?>">Baja</a>
      </div>
      <div class="tk-filter-sep"></div>
      <div class="tk-filter-group">
        <a class="tk-filter-pill<?= $fSort==='desc'?' active':'' ?>" href="<?= tkUrl(['sort'=>'desc','page'=>1]) ?>" title="Más recientes primero">
          Más reciente
        </a>
        <a class="tk-filter-pill<?= $fSort==='asc'?' active':'' ?>" href="<?= tkUrl(['sort'=>'asc','page'=>1]) ?>" title="Más antiguos primero">
          Más antiguo
        </a>
      </div>
      <div class="tk-filter-sep"></div>
      ...
      <div class="tk-filter-dropdown">
        ...
        <div class="tk-dropdown-menu">
          <a class="tk-dropdown-item<?= $fCat===''?' active':'' ?>" href="<?= tkUrl(['categoria_id'=>'']) ?>">Todas</a>
          <?php foreach ($categorias as $cat): ?>
            <a class="tk-dropdown-item<?= (string)$fCat===(string)$cat['id']?' active':'' ?>"
               href="<?= tkUrl(['categoria_id'=>$cat['id']]) ?>">
      ...
      <?php endif; ?>
      <?php if ($isAdmin && !empty($departamentos)): ?>
      <div class="tk-filter-sep"></div>
      <div class="tk-filter-dropdown">
        <button type="button" class="tk-dropdown-btn">
          <span class="material-symbols-outlined" style="font-size:16px">corporate_fare</span>
          <span><?= $fDepto ? htmlspecialchars(array_column($departamentos,'nombre','id')[$fDepto] ?? 'Departamento') : 'Departamento' ?></span>
          <span class="material-symbols-outlined">expand_more</span>
        </button>
        <div class="tk-dropdown-menu">
          <a class="tk-dropdown-item<?= $fDepto===''?' active':'' ?>" href="<?= tkUrl(['departamento_id'=>'']) ?>">Todos</a>
          <?php foreach ($departamentos as $dep): ?>
            <a class="tk-dropdown-item<?= (string)$fDepto===(string)$dep['id']?' active':'' ?>"
               href="<?= tkUrl(['departamento_id'=>$dep['id']]) ?>">
      ...
      <?php endif; ?>
      <a href="index.php?controller=Ticket&action=listar" class="tk-reset-btn" id="tk-reset" title="Limpiar filtros">
    ...
    <div class="tk-table-wrap" style="margin-top:16px">
        ...
        <table class="tk-table">
          ...
          <tbody>
            <?php if (empty($tickets)): ?>
              <tr><td colspan="8" style="text-align:center;padding:32px;color:var(--text-secondary)">No se encontraron tickets.</td></tr>
            ...
            ?>
              <tr data-href="index.php?controller=Ticket&action=detalle&id=<?= $tId ?>"
                ...
                <td><button class="tk-action-btn" data-id="<?= $tId ?>"><span class="material-symbols-outlined">more_vert</span></button></td>
              </tr>
      ...
      <div class="tk-pagination">
        ...
        <div class="tk-pagination-btns">
          <?php
            $qBase = array_filter(['controller'=>'Ticket','action'=>'listar','estado'=>$fEstado,'prioridad'=>$fPrioridad,'q'=>$fQ,'categoria_id'=>$fCat,'departamento_id'=>$fDepto,'sort'=>$fSort],fn($v)=>$v!=='');
          ?>
...
<!-- Menú contextual (compartido, reposicionado por JS) -->
